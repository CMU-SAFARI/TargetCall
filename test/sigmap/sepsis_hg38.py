...
human_path = datadir + "refs/human.fasta "
human_index_path = datadir + "refs/sigmap_indices/human "
...
end_time = time.time()
print("SEPSIS-hg38 Human Sigmap execution time: %s seconds " % (end_time - start_time))

start_time = time.time()
os.system("python " + sigmap_script_path + covid_fast5_path + " " + sam_dir_path + "bacteria_" + ' ' +  human_path + ' ' + human_index_path + ' ' + sigmap_exe)
end_time = time.time()
print("SEPSIS-hg38 bacteria Sigmap execution time: %s seconds " % (end_time - start_time))
