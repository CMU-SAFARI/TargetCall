...
end_time = time.time()
print("COVID Human BaseConv execution time: %s seconds " % (end_time - start_time))

start_time = time.time()
os.system("python " + bonito_script_path + covid_fast5_path + " " + fastq_dir_path + "covid_" + ' ' +  model_name)
end_time = time.time()
print("COVID Covid BaseConv execution time: %s seconds " % (end_time - start_time))
...
end_time = time.time()
print("VIRAL bacteria BaseConv execution time: %s seconds " % (end_time - start_time))

start_time = time.time()
os.system("python " + bonito_script_path + simulated_virus_fast5_path + " " + fastq_dir_path + "virus_" + ' ' +  model_name)
end_time = time.time()
print("VIRAL virus BaseConv execution time: %s seconds " % (end_time - start_time))
...
os.system("mkdir -p " + fastq_dir_path)
sam_dir_path = datadir + '/sepsis/' + model_name + '/sam/'
os.system("mkdir -p " + sam_dir_path)
readids_dir_path = datadir + '/sepsis/' + model_name + '/readids/'
os.system("mkdir -p " + readids_dir_path)
filtered_fastq_dir_path = datadir + '/sepsis/' + model_name + '/filtered-fastq/'
os.system("mkdir -p " + filtered_fastq_dir_path)
...
end_time = time.time()
print("SEPSIS Sepsis-bacteria BaseConv execution time: %s seconds " % (end_time - start_time))
...
end_time = time.time()
print("SEPSIS dataset SimCheck execution time: %s seconds " % (end_time - start_time))

# RUN extract filtered reads 
start_time = time.time()
os.system("python " + extract_read_ids_path + sam_dir_path + " " + readids_dir_path + " sepsis")
end_time = time.time()
print("SEPSIS dataset extract read ids execution time: %s seconds " % (end_time - start_time))
...
end_time = time.time()
print("SEPSIS basecalling after filter execution time: %s seconds " % (end_time - start_time))
