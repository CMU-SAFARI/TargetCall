...
for idx, model in enumerate(models):
    ...
    if model == "default":
        for line in out_file:
            if line.startswith("COVID Human basecalling"):
                index = line.strip().split(":")
                value = index[-1].strip().split(" ")
                execution_times[0][0] += float(value[0].strip())
                execution_times[0][2] += float(value[0].strip())
            elif line.startswith("COVID Covid basecalling"):
                index = line.strip().split(":")
                value = index[-1].strip().split(" ")
                execution_times[0][2] += float(value[0].strip())
            elif line.startswith("VIRAL bacteria basecalling"):
                index = line.strip().split(":")
                value = index[-1].strip().split(" ")
                execution_times[0][1] += float(value[0].strip())
            elif line.startswith("VIRAL virus basecalling"):
                index = line.strip().split(":")
                value = index[-1].strip().split(" ")
                execution_times[0][1] += float(value[0].strip())
            elif line.startswith("SEPSIS Sepsis-bacteria basecalling"):
                index = line.strip().split(":")
                value = index[-1].strip().split(" ")
                execution_times[0][0] += float(value[0].strip())
        ...
        for line in out_file:
            ...
            if line.startswith("COVID"):
                ...
                if line.startswith("COVID Human BaseConv"):
                    execution_times[idx][0] += float(value[0].strip())
